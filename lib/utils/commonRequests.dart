...
class XSerApi {
  ...
  /// d : 传入data
  /// t : 保存密码标识
  static Future<Map<String,dynamic>> Studentlogin(Map<String,dynamic> d,bool t)async{
    Map<String,dynamic> res = await HttpUtils.request('/login/student',data: d);
...
//缓存的MyXS数据
   static Future<Map<String, dynamic>> getMyXS()async{
    ...
    if(!await checkLoginbyDB()){
      data={'RealName':'请先登录','balance':0,'notifications':0,'weather':"0",'dd90':0,'grade':"测试班级",'day':'周一','week':'第一周','date':'1月1日','weathericon':'sunny'};
    }else{
      //请求信息
      data={'RealName':'王二狗','balance':0,'notifications':0,'weather':"24℃",'dd90':0,'grade':"测试班级",'day':'周一','week':'第一周','date':'1月1日','weathericon':'sunny'};
    }
    if(data.isNotEmpty){
    await sqlLite.open();
    print("更新myxs数据");
...
/*
  ...
  static Future<Map<String, dynamic>> getUserDetail()async{
    ...
    detail = await HttpUtils.request('/user/userinfo',data: {'secret':"${secret}"});
    return detail;
  }
  ...
  static Future<bool> checkLogin() async {
    ...
    if(loginstate[0]["login"]==0){
      ...
      return false;
        ...
        Map<String, dynamic> status = await HttpUtils.request('/login/status',data: {'secret':"${secret}"});
        if(status["code"]==200){
          print("登陆有效");
          return true;
        }else{
  ...
  static Future<Map<String, dynamic>> getArticle({String id})async{
    ...
    return await HttpUtils.request('/article/',method: 'get',data:{"id":id});
  }
  ...
  static Future<Map<String, dynamic>> getPublicContent(int page)async{
    Map<String, dynamic> detail = await HttpUtils.request('/publiccontent/comments/bypage',data: {'page':"${page.toString()}"});
    return detail;
  }
    ...
    if(loginstate[0]["login"]==0){
      ...
      return false;
        ...
        if(dt[0]["rememberPwd"]==1){
          ...
          Map<String,dynamic> result=await Studentlogin({"user":"${dt[0]["Account"]}","password":"${dt[0]["password"]}","method":"student"}, true);
          if(result["code"]==200){
            print("重新登陆成功");
            return true;
          }else{
  ...
  //主页文章更新 概览 每条只有两个
  static Future<Map<String,dynamic>> homeArticle() async{
    Map<String,dynamic> data=await HttpUtils.request('/publiccontent/homepagebrief/',method: 'get');
  }
...
//最新文章 - alllatest
//校务公开 - schoolaffair
  static Future<Map<String,dynamic>> getArticleList(String type,int page) async{
    String a;
    switch(type){
      case "通知公告": a='board';break;
      case "最新文章": a='alllatest';break;
      case "校务公开": a='schoolaffair';break;
      default:break;
    }
    return await HttpUtils.request('/publiccontent/${a}/bypage/',method: 'get',data:{"page":page.toString(),'n':"20"});
  }
}